...
function Home() {
    const handleSubmit = async() => {
        try {
            ...
            if (response.code) {
                ...
                statusEl.innerHTML = `Success! Your short URL is: <a id="shortenedURLHyperlink" href="${shortenedURL}" target="_blank">${shortenedURL}</a>`;
            } else {
                statusEl.classList.add("error");
                statusEl.textContent = `An error occured! ${response.cause}`;
                console.log("Request:")
                console.log(request)
    ...
    return ( <
        ...
        <
        Link to = "/statistics"
        id = "viewStatisticsRedirect" > View Statistics < /Link> <
        Link to = "/admin"
        id = "adminRedirect" > Admin Portal < /Link>
